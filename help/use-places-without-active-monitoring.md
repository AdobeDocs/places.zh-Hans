...
>[!TIP]
>
>如果您的应用程序用例需要活动区域监视，请使 [用地点监视器扩展阅读](/help/places-ext-aep-sdks/places-monitor-extension/places-monitor-extension.md)。

要使用无活动区域监视的地点，请执行以下操作：

## 1.收集用户位置

应用程序开发人员必须使用Google Play Services(Android)提供的 `CoreLocation.framework` (iOS)或API来收集设备的当 `Location` 前位置。

有关详细信息，请参阅以下文档：

- [CoreLocation](https://developer.apple.com/documentation/corelocation) (Apple)
- [Google Play Services中的位置API](https://developer.android.com/training/location) (Google)
...
以下代码示例显示了对在回调中提供的结果的 `getNearbyPointsOfInterest:limit:callback:errorCallback:`处理， `NSArray<ACPPlacesPoi *> *`
...
以下代码示例显示了对在回调中提供的结果的 `getNearbyPoints(_ ofInterest: CLLocation, limit: UInt, callback: (([ACPPlacesPoi]?) -> Void)?, errorCallback: ((ACPPlacesRequestError) -> Void)?)`处理， `[ACPPlacesPoi]`
...
除了在SDK中触发Places条目事件外，由于触发条目事件，定义POI的所有数据都可由SDK的其余部分在Launch中使 `data elements` 用。 借助Launch `rules`，您可以将地点数据动态附加到SDK处理的传入事件。 例如，您可以附加用户所在POI的元数据，并将该数据作为上下文数据发送到Analytics。

有关详细信息，请参 [阅将地点与其他Adobe解决方案结合使用](/help/use-places-with-other-solutions/use-places-with-other-solutions.md)。
